...
class ActorNetwork(nn.Module):
    ...
    def forward(self, state):
        ...
        x = self.bn2(x)
        x = F.relu(x)
        x = (T.tanh(self.mu(x)) + 1) / T.sum(T.tanh(self.mu(x)) + 1)
        return x
...
class DDPGAgent(object):
    ...
    def __init__(self, alpha, beta, input_dims, tau, env, gamma=0.99, 
                ...
                layer2_size=300, batch_size=64, training_mode=True):
#         self.name = "DDPG"
...
#         print("Noise: ", n)
        ...
        if self.training_mode:
            mu_prime += T.tensor(n, dtype=T.float).to(self.actor.device)
        mu_prime = T.tanh(mu_prime) + 1
#         print("mu_prime1:", mu_prime)
        mu_prime = mu_prime / T.sum(mu_prime)
#         print("mu_prime2:", mu_prime)
        self.actor.train()
